...
class LinkPredict(nn.Module):
    def __init__(self, g, features, in_dims, h_dim, num_rels, num_bases=-1,
        ...
        for i, ntype in enumerate(g.ntypes):
            h_dict[ntype] = features[i]

        max_length = max([feat.shape[1] for feat in h_dict.values()])

        self.h2h = simpleHGN(g, h_dict, max_length, args.n_hid, 3, [1, 1, 1]).to(device)
        print(self.h2h)
...
def run_model_DBLP(args):
    ...
    if True:
        ...
        early_stopping = EarlyStopping(patience=args.patience, verbose=True,
                                       save_path='/local/scratch/hcui25/Project/xin/H2H/LP/simple-HGN/checkpoint/rgcn_checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers))
    ...
    for test_edge_type in dl.links_test['data'].keys():
        ...
        net.load_state_dict(torch.load(
            '/local/scratch/hcui25/Project/xin/H2H/LP/simple-HGN/checkpoint/rgcn_checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers)))
        net.eval()
        test_logits = []
        with torch.no_grad():
